...
class _NormalizationMixin:
    ...
    **Stats Override Preservation:**
    ...
    def _apply_transform(
        ...
        action_stats = self._tensor_stats.get(ACTION, {})
        if norm_mode == NormalizationMode.MEAN_STD:
            mean = stats.get("mean", None)
            std = stats.get("std", None)
            action_mean = action_stats.get("mean", None)
            ...
            history_action_n = (tensor.size()[-1] - std.size()[0]) / std.size()[0]
            if history_action_n > 0 and key == "observation.state":
                mean = torch.cat([action_mean.repeat(int(history_action_n)), mean], dim=0)
                std = torch.cat([action_std.repeat(int(history_action_n)), std], dim=0)
        ...
        if norm_mode == NormalizationMode.MIN_MAX:
            ...
            history_action_n = (tensor.size()[-1] - min_val.size()[0]) / min_val.size()[0]
            if history_action_n > 0 and key == "observation.state":
                max_val =max_val.repeat(int(history_action_n)+1)
                min_val =min_val.repeat(int(history_action_n)+1)
